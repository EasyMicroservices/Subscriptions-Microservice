...
using EasyMicroservices.SubscriptionsMicroservice.Contracts;
using EasyMicroservices.SubscriptionsMicroservice.Interfaces;
using EasyMicroservices.SubscriptionsMicroservice.Database;
using EasyMicroservices.SubscriptionsMicroservice.Interfaces;
using EasyMicroservices.SubscriptionsMicroservice;
using EasyMicroservices.SubscriptionsMicroservice.Contracts.Common;

namespace EasyMicroservices.SubscriptionsMicroservice.WebApi
{
    public class Program
    {
        ...
        public static async Task Main(string[] args)
        {
            ...
            builder.Services.AddScoped((serviceProvider) => new DependencyManager().GetContractLogic<SubscriptionEntity, AddSubscriptionRequestContract, UpdateSubscriptionRequestContract, SubscriptionContract>());
            builder.Services.AddScoped((serviceProvider) => new DependencyManager().GetContractLogic<SubscriptionUserEntity, AddSubscriptionUserRequestContract, UpdateSubscriptionUserRequestContract, SubscriptionUserContract>());
            builder.Services.AddScoped((serviceProvider) => new DependencyManager().GetContractLogic<SubscriptionParameterEntity, AddSubscriptionParameterRequestContract, UpdateSubscriptionParameterRequestContract, SubscriptionParameterContract> ());
            builder.Services.AddScoped((serviceProvider) => new DependencyManager().GetContractLogic<SubscriptionParameterValueEntity, AddSubscriptionParameterValueRequestContract, UpdateSubscriptionParameterValueRequestContract, SubscriptionParameterValueContract>());
            ...
            builder.Services.AddTransient(serviceProvider => new SubscriptionContext(serviceProvider.GetService<IDatabaseBuilder>()));
            ...
            using (var scope = app.Services.CreateScope())
            {
                ...
                await service.Initialize("Subscription", config.GetValue<string>("RootAddresses:WhiteLabel"), typeof(SubscriptionContext));
            }
        ...
        {
            using (var context = new SubscriptionContext(new DatabaseBuilder()))
            {
